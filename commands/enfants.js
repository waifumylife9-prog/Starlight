...
const DUREE_STADE = 24 * 60 * 60 * 1000;

const STADES = {
    bebe: { emoji: '👶', label: 'Bébé', prochain: 'enfant' },
    enfant: { emoji: '🧒', label: 'Enfant', prochain: 'adulte' },
    adulte: { emoji: '👩', label: 'Adulte', prochain: null },
};
        ...
        try {
            ...
            if (grossesses.length > 0) {
                ...
                for (const maman of grossesses) {
                    ...
                    texteGrossesses += `🎯 <t:${Math.floor(new Date(maman.finGrossesse).getTime() / 1000)}:R>\n\n`;
                }
            ...
            if (enfants.length > 0) {
                ...
                for (const enfant of enfants) {
                    ...
                    if (stade.prochain && finStade) {
                        ...
                        texteEnfants += `🔓 <t:${Math.floor(finStade / 1000)}:R>\n`;
                    }
